...
def dodanie_pytanie_dziedzina(flag_2 = 1):
    flag = False
    top_list()
    try:
        if flag_2:
            print("\nWybierz dziedzinę do której chcech dodać pytanie lub [0] "
                "dla stworzenia nowej:")
            for x in range(len(TOPICS_LIST)):
                print("({})-> {}".format(x+1,TOPICS_LIST[x]))
        choice = int(input("Wprowadź numer tematu lub 0:"))
        if choice == 0:
            with open(os.path.join(sys.path[0],'Pytania','TEMATY.json'), 'r+',
                    encoding='utf8') as f:
                TOPICS_LIST.append(input('\nWprowadź nazwę nowej dziedziny:\n'))
    ...
    if not flag:
        ...
        dodanie_pytanie_dziedzina(0)# od nowa jeżeli flaga = True
...
def add_question(filename, pytanie = None, pyt_z_odp = None):
    try:
        if pytanie:
            ...
            play = input()
        if play.lower() not in ['0', 'n', 'nie','t','tak','']:
            raise ValueError("Poprawne wartości: Enter/Tak/T lub 0/Nie/N")
        if play.lower() in ['0', 'n','nie'] :
            return print('\n{:_^80s}\n'.format("DANE ZAPISANO"))
        if not pytanie:
            nowe_pytanie = {}               
            nowe_pytanie = {"pytanie" : 
                input("Proszę o wprowadzenie pytania:\n")}
        ...
        if not pyt_z_odp :
            try:
                ...
                return add_question(filename, nowe_pytanie) 
                    # zaczyna od razu od miejsca gdzie rzucono ValueError
        else:
        ...
        try:
            ...
            return add_question(filename,True, nowe_pytanie) 
                # zaczyna od razu od miejsca gdzie rzucono ValueError
        ...
        try:    
            with open(filename, "r+", encoding='utf8') as file:
                if os.stat(filename).st_size == 0:
                    ...
                    print("Dodano pytanie.Ilość obecnych pytań: {}".
                        format(data["Ilość pytań"]))
        except IOError:
